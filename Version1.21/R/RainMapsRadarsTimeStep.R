...
#' Function which visualises radar rainfall depths for chosen TIMESTEP in configuration file.
...
#' for ExtraDeg should be specified (decimal degrees). 
...
#' downloaded Google Maps. Because also axes and legend are plotted, it is advised to use 
#' e.g. 1450 times 1450 pixels. Then the Google Map will remain approximately 1280 times 1280 pixels. 
#' Using higher values is not a problem (e.g. 2000). In this way it is tried to get the 
#' highest possible resolution. For OpenStreetMap the maps may reach resolutions of 1500 - 2000 pixels. 
#' Hence, using FigWidth and FigHeight of 2000 pixels or higher is advised. The OpenStreetMap itself 
...
#' @param OSMBottom Latitude in decimal degrees (WGS84) for bottom side of the area for which rainfall depths are 
#' to be plotted (for OpenStreetMap & Stamen Maps only).
#' @param OSMLeft Longitude in decimal degrees (WGS84) for left side of the area for which rainfall depths are to 
#' be plotted (for OpenStreetMap & Stamen Maps only). 
#' @param OSMRight Longitude in decimal degrees (WGS84) for right side of the area for which rainfall depths are 
#' to be plotted (for OpenStreetMap & Stamen Maps only). 
...
#' @param ScaleBottomTimeStep ScaleBottomDaily Lowest class starts at this threshold (minimum rainfall 
#' accumulation (mm) to be plotted). Using a value clearly above 0 mm can save a lot of computation time if 
#' the polygons belonging to values below the threshold are not plotted.
#' @param ScaleHigh ScaleHigh Highest value per class interval, i.e. the highest legend breaks, if these are 
#' manually chosen.  Please note that in case of x values in ColoursNumber, ScaleHigh should also contain x 
#' values.
...
#' LegendSize=LegendSize,LegendTitleRadarsTimeStep=LegendTitleRadarsTimeStep,
...
LatText,LegendSize,LegendTitleRadarsTimeStep,LonLocation,LonText,ManualScale,MapBackground,
OSMBottom,OSMLeft,OSMRight,OSMScale,OSMTop,OutputFileType,PathRadarRainfallDepth,PERIOD,
...
XMiddle,YMiddle)
{
	...
	# Output time interval of radar data in min:
	OutputTimeInterval <- 15
		
	# Read files (meant to process radar data from one 24-hour period):
	Files <- list.files(path = FolderRadarRainMapsTimeStep, all.files=FALSE, full.names=TRUE, recursive=FALSE, pattern=".nc")
	Files <- Files[which(file.info(Files)$size>0)]

	# Check whether all files are present or contain data:
	NumberInputTimeIntervals <- 1440/InputTimeInterval
	if (length(Files)!=NumberInputTimeIntervals)
	{
		print("Number of input files (with data) is not correct! There are missing data.")
		stop()
	}
	...
	Conversion <- OutputTimeInterval/InputTimeInterval
	for (FileNr in 1:(length(Files)/Conversion))
	{

		# Read radar rainfall depths:
		# Open NetCDF file from first 5 min:
		ncFILE <- nc_open(Files[(FileNr-1)*Conversion+1],verbose=F)
		Data1 <- c(ncvar_get(ncFILE,varid=PathRadarRainfallDepth))
		Data1 <- Data1[!is.na(Data1)]
		nc_close(ncFILE)

		# Open NetCDF file from second 5 min:
		ncFILE <- nc_open(Files[(FileNr-1)*Conversion+2],verbose=F)
		Data2 <- c(ncvar_get(ncFILE,varid=PathRadarRainfallDepth))
		Data2 <- Data2[!is.na(Data2)]
		nc_close(ncFILE)

		# Open NetCDF file from third 5 min:
		ncFILE <- nc_open(Files[(FileNr-1)*Conversion+3],verbose=F)
		Data3 <- c(ncvar_get(ncFILE,varid=PathRadarRainfallDepth))
		Data3 <- Data3[!is.na(Data3)]
		nc_close(ncFILE)

		# Accumulate 5-min rainfall depths to 15-min rainfall depths:
		...
		Value <- ToPolygonsRain(Data=Data[CondMap])
		...
		FigFilename <- paste(FolderFigures,"/",FigFileRadarsTimeStep,EndDateTime,".",OutputFileType,sep="")
		...
		if (PlotLocation=="yes")
		{
			...
			Title <- paste(paste(TitleRadars,"; ",ExtraText,sep=""),DateTime,paste("+",LocationOnMap,sep=""),sep="\n")
			...
			PointValue <- ReadRainLocation(CoorSystemInputData=CoorSystemInputData,dataf=Data,
