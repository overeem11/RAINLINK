...
#' @description Function which visualises daily link-based rainfall depths. Requires interpolation grid and file with polygons of pixels. Daily rainfall depths are computed irrespective of the number of available files. If, for instance, only one out of 96 files is available, the daily rainfall depth is still computed and visualised. The data availability is plotted as a percentage in the title caption of the graph.
...
#' @examples
#' RainMapsLinksDaily(20110911) # To plot daily link rainfall map ending at 11 September 2011.
#' 

RainMapsLinksDaily <- function(DateMap)
{
	...
	# Select files with date "DateMap" in file name:
	sel <- grep(DateMap,Files)
	Files <- Files[sel]
	...
	for (FileNr in 1:length(Files))
	{
		...
		R_day[1:NrGridPoints] <- R_day[1:NrGridPoints] + Data$RainIntensity[1:NrGridPoints]
	}
	# Convert rainfall intensities to rainfall depths:
	R_day <- R_day / ConversionDepthToIntensity
	...
	{	
		...
		tempdata <- unique(data.frame(cbind(TEMP$XStart,TEMP$YStart,TEMP$XEnd,TEMP$YEnd)))
		names(tempdata) <- c("XStart","YStart","XEnd","YEnd")
		rm(TEMP)
	...
	{		
		...
		xend=data_coor[,3],yend=data_coor[,4]),data=data_coor,alpha=alpha_links_daily,
	...
	FigFilename <- paste(FolderFigures,"/",FigFileLinksDaily,EndDateTimeFile,".jpeg",sep="")
