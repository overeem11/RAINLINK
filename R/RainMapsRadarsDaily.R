...
#' the plotted region, a positive number for ExtraDeg should be specified (decimal degrees). 
...
#' @param GoogleLocDegSpecified If GoogleLocDegSpecified is "yes" then the specified location in decimal degrees 
...
#' @param OSMBottom Latitude in decimal degrees (WGS84) for bottom side of the area for which rainfall depths are to 
#' be plotted (for OpenStreetMap & Stamen Maps only).
#' @param OSMLeft Longitude in decimal degrees (WGS84) for left side of the area for which rainfall depths are to be 
#' plotted (for OpenStreetMap & Stamen Maps only). 
#' @param OSMRight Longitude in decimal degrees (WGS84) for right side of the area for which rainfall depths are to 
#' be plotted (for OpenStreetMap & Stamen Maps only). 
...
#' LegendSize=LegendSize,LegendTitleRadarsDaily=LegendTitleRadarsDaily,
...
#' PathRadarRainfallDepth=PathRadarRainfallDepth,PERIOD=PERIOD,
...
RainMapsRadarsDaily <- function(AlphaPlotLocation,AlphaPolygon,AutDefineLegendTop,
BBoxOSMauto,ColoursNumber,ColourPlotLocation,ColourPlotLocationText,ColourScheme,
ColourType,ColourHighestClass,CoorSystemInputData,DateMap,ExtraDeg,ExtraText,
FigFileRadarsDaily,FigHeight,FigWidth,FileGrid,FileNameRadarDaily,FilePolygonsGrid,
FolderFigures,FolderRadarRainMapsDaily,FontFamily,GoogleLocDegSpecified,GoogleLocLat,
...
LatText,LegendSize,LegendTitleRadarsDaily,LonLocation,LonText,ManualScale,MapBackground,
OSMBottom,OSMLeft,OSMRight,OSMScale,OSMTop,OutputFileType,PathRadarRainfallDepth,PERIOD,PlotLocation,
PixelBorderCol,PlotBelowScaleBottom,ScaleBottomDaily,ScaleHigh,ScaleLow,
ScaleTopDaily,SizePixelBorder,SizePlotLocation,SizePlotTitle,StamenMapType,
StamenZoomlevel,SymbolPlotLocation,TIMESTEP,TimeZone,TitleRadars,XMiddle,YMiddle)
{
	...
	FigFilename <- paste(FolderFigures,"/",FigFileRadarsDaily,EndDateTime,".",OutputFileType,sep="")
