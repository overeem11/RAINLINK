...
OutlierFilterMinMaxRSL <- function(Data,F,FilterThreshold) 
{
	...
	# In case F is not available:.
	if (is.null(F))
	{
		print("F is not available, because function WetDryNearbyLinkApMinMaxRSL has not been executed.
		Therefore, the outlier filter cannot be applied.")
	}
